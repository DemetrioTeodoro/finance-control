...
**Pagamento de fatura de cartão não deve contar como despesa duas vezes (14/09/2026)**: quando o usuário importa tanto a fatura do cartão (compras individuais com `creditCardId`) quanto o extrato bancário (que inclui o pagamento dessa fatura como uma despesa comum com `accountId`), o mesmo valor entrava duas vezes no somatório de despesas. Resolvido adicionando `Transaction.paidCreditCardId` (campo novo, distinto do `creditCardId` já existente — este último marca compras da fatura, sem afetar saldo; `paidCreditCardId` marca uma despesa de conta como "isto é o pagamento da fatura deste cartão"). A transação continua afetando o saldo da conta normalmente (`createTransaction`/`updateTransaction` em `src/services/transaction.ts` validam que só pode ser usado com `type: "expense"` e que o cartão pertence ao usuário), mas `getReportSummary`, `getExpensesByCategory`, `getExpensesByAccount` (`src/services/report.ts`) e os totais de despesa do dashboard (`src/services/dashboard.ts`, incluindo `getMonthlyEvolution`) passaram a filtrar `paidCreditCardId: null` para excluir esse valor do somatório — sem isso, o saldo da conta ficaria correto, mas o "total de despesas" ficaria inflado. A marcação é manual (select no form de criar/editar transação, `src/app/(dashboard)/transacoes/`), feita ao editar a transação de pagamento já importada do extrato; não há detecção automática durante o import OFX, pois dependeria de heurística de texto frágil sobre a descrição do banco.

**Contas fixas (18/09/2026)**: adicionado checklist mensal de contas recorrentes (aluguel, internet, energia etc.), em `/contas-fixas`. Dois modelos novos: `RecurringBill` (nome, valor esperado opcional, dia de vencimento, categoria opcional, `active`) e `RecurringBillPayment` (um registro por mês pago, `@@unique([recurringBillId, year, month])`, com `transactionId` opcional). "Pago" é resolvido de duas formas, não mutuamente exclusivas: (1) automática — `checkRecurringBillsForReminders` (`src/services/recurring-bill.ts`), chamada 1x/dia pelo Vercel Cron (`GET /api/cron/recurring-bills`, agendado em `vercel.json`, protegido por `CRON_SECRET`), casa a conta fixa com uma `Transaction` do tipo `expense` na mesma categoria dentro do mês corrente e cria o `RecurringBillPayment` com `transactionId` preenchido; (2) manual — botão "Confirmar pagamento" na UI, que cria o mesmo registro sem `transactionId` (para casos sem categoria definida ou pagos fora do fluxo de transações rastreadas). Mesma limitação já registrada para `paidCreditCardId`: o casamento automático depende da categoria ser específica daquela conta fixa (ex.: categoria "Aluguel" só usada para o aluguel) — não há heurística de texto sobre a descrição, por ser frágil. Lembrete por e-mail (1 dia antes e no dia do vencimento, dia clampado ao último dia do mês em meses mais curtos, mesma lógica de `resolveInvoiceCycleKey`/`getInvoiceCycleRange` do cartão de crédito) enviado via API HTTP do Resend (`src/lib/email.ts`, sem SDK novo — usa `fetch` direto), lido de `RESEND_API_KEY`/`RESEND_FROM_EMAIL`. Escolhido e-mail em vez de push/WhatsApp após discussão com o usuário: WhatsApp oficial exige verificação de negócio (inviável para projeto pessoal grátis) e push web exigiria manifest/service worker/tabela de subscriptions que o usuário preferiu evitar.

**Contas fixas — checklist de histórico e vínculo manual de transação (20/09/2026)**: a página `/contas-fixas` deixou de mostrar só o mês corrente e passou a ser uma tabela (`getRecurringBillsWithHistory`, últimos 6 meses) com uma coluna por mês — o usuário pediu para conseguir verificar meses anteriores num só lugar, em vez de só o mês atual. Cada célula (`recurring-bill-month-cell.tsx`) é clicável e abre um dialog: se pendente, permite confirmar com ou sem vincular uma transação específica daquele mês (`getExpenseTransactionOptionsByMonth`, lista as despesas do mês para escolher no select); se já paga, mostra a transação vinculada (quando houver) e permite desfazer. `confirmRecurringBillPayment` (`src/services/recurring-bill.ts` e `src/actions/recurring-bill.ts`) passou a aceitar `transactionId` opcional, validando que a transação pertence ao usuário antes de vincular.

**Contas fixas — remoção do casamento automático por categoria, vínculo direto na transação (21/09/2026)**: o casamento automático do cron (categoria + mês, sem checar valor) causava falso positivo — qualquer despesa na mesma categoria no mês marcava a conta como paga, mesmo não sendo o pagamento real. Removido (`findMatchingTransaction` e o bloco de auto-match em `checkRecurringBillsForReminders` não existem mais; o cron só verifica se já está pago e manda o e-mail de lembrete, nunca marca como pago). Em vez disso, adicionado `Transaction.paidRecurringBillId` (mesmo padrão de `Transaction.paidCreditCardId`): no formulário de criar/editar transação (`src/app/(dashboard)/transacoes/`), um select "Esta despesa é o pagamento de uma conta fixa?" vincula a transação direto à conta fixa — vínculo explícito, sem heurística. `getRecurringBillsWithHistory` agora resolve "pago" combinando duas fontes, não mutuamente exclusivas: (1) alguma `Transaction` com `paidRecurringBillId` apontando pra essa conta dentro do mês (vínculo pelo formulário de transação, ou pelo checklist — `confirmRecurringBillPayment` com `transactionId` agora seta esse campo na transação em vez de gravar o vínculo no `RecurringBillPayment`); (2) confirmação manual sem transação (`RecurringBillPayment`, sem o campo `transactionId` que existia antes — pago em dinheiro/fora do fluxo rastreado). `categoryId` na conta fixa continua existindo, mas só para exibição (cor na listagem) — não é mais usado por nenhuma lógica de detecção de pagamento. Migration `20260921150000_recurring_bill_transaction_link` migrou o único registro real que existia com `RecurringBillPayment.transactionId` preenchido para o novo `Transaction.paidRecurringBillId`, sem perda de dado.

**Contas fixas — filtro de período (21/09/2026)**: o checklist tinha uma quantidade fixa de meses (6, depois um dropdown limitado a 24). Trocado pelo mesmo padrão de `src/app/(dashboard)/relatorios/report-filters.tsx`: presets (3/6/12/24 meses) + período personalizado com `DateInput` de "De"/"Até", numa barra de filtro própria abaixo do cabeçalho (em vez de espremido do lado do botão "+ Nova conta fixa", que ficava com alturas/alinhamento diferentes). `resolveRecurringBillPeriod`/`resolveRecurringBillMonths` (`src/services/recurring-bill.ts`) espelham `resolveReportPeriod`/`resolveReportRange` — período personalizado usa `parseLocalDate`, ignora o dia (só ano/mês importam pro checklist) e é limitado a 60 meses (`CUSTOM_PERIOD_MAX_MONTHS`) pra não gerar uma tabela absurdamente larga. `getRecurringBillsWithHistory` deixou de calcular os meses internamente (recebia `monthsCount`) e passou a receber a lista de `{year, month}` já resolvida — quem decide o range é sempre `resolveRecurringBillMonths`, chamado uma vez em `page.tsx`.

**Import OFX sem duplicar transações (24/09/2026)**: antes, importar o mesmo extrato/fatura duas vezes (ou dois arquivos com períodos sobrepostos) duplicava as transações e, no caso de conta, aplicava o valor ao saldo de novo — obrigando o usuário a esperar o mês fechar para importar. Adicionado `Transaction.externalId` (FITID do OFX, nulo em transações manuais), com `@@unique([accountId, externalId])` e `@@unique([creditCardId, externalId])`. O `py-api` envia o FITID como `idExterno` (o `ofxparse` já rejeita OFX sem FITID), e `bulkCreateAccountTransactions`/`bulkCreateCreditCardTransactions` (`src/services/transaction.ts`) descartam, dentro do mesmo `prisma.$transaction`, as que já existem no recurso de destino ou se repetem no lote — o delta de saldo da conta é calculado **só sobre as transações realmente criadas**. A rota bulk retorna `{ count, skipped }` e o toast mostra quantas eram novas e quantas foram ignoradas. Os índices únicos protegem contra dois imports concorrentes (P2002 → 409). Limitação: transações importadas antes desta mudança não têm `externalId`, então reimportar um período que já tinha sido importado antes dela duplica essas transações — na transição, importar só a partir do dia seguinte ao último import antigo.
